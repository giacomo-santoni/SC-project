...
The data have been rearranged to have a format consistent with the simulated ones and then labelled. The labelling criterion is based on the ratio $`\frac{N_{inner photons}}{N_{total photons}}`$: if it's larger than 0.1, the camera is considered dazzled (_D_), and a 1 is assigned to it, otherwise, it is non-dazzled (_ND_), and labelled with 0. This criterion was chosen to account for situations like the third case presented in Section 1.2.1 when the particle starts to emit before the camera and continues inside. So, if we had looked only at the absolute number of photons, we would have discarded a camera that could be useful for the reconstruction.

These adjustments were made since these data have to represent only the state of the camera, i.e., dazzled/non-dazzled. So, in the end, an array of 0 and 1 was obtained.

## 2.3 Dataset features and rearrangements
This dataset is highly imbalanced towards _ND_ cameras, with a distribution of 99.7% _ND_ - 0.3% _D_. So, with this kind of data, a neural network would be optimal in finding the _ND_ cameras, but only because they are in higher amount. For this reason, an augmentation to the _D_ cameras was applied, increasing their abundance to 35% with respect to the total number of events.
...
Then, the dataset was split into three subsets: a training dataset of $\sim 10^5$ events, a validation dataset of $\sim 10^3$ events, and a test dataset of $\sim 10^4$ events. The augmentation dataset was applied on the training and validation datasets. 
