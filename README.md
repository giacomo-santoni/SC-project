...
This dataset is highly imbalanced towards $\overline{D}$ cameras, with a distribution of 99.7% $\overline{D}$ - 0.3% $D$. So, with this kind of data, a neural network would be optimal in finding the $\overline{D}$ cameras, but only because they are in larger amounts. For this reason, an augmentation to the $D$ cameras was applied, increasing their abundance to 35% with respect to the total number of events.
